...
- 명령: `npm test`
- 실제 결과: 20개 테스트 통과. 결정적 분석, 한국어 신호와 근거, 프롬프트 인젝션의 일반 텍스트 처리, 표시·신호·도구 결과 및 문맥형 비밀값 마스킹, 개인정보·에이전트 변경 게이트, 사례·버전 충돌, 동의를 되살리지 않는 단조 증가 실행 취소, 오래된 분석·단계 차단, 체크리스트 진행 상태 보존, WebMCP 계약을 확인했습니다.
- 명령: `npm run build`
