...
| 1:20–1:35 | Call `get_case_summary({})`; show case ID, numeric version, and sanitized signal evidence. | “The agent receives structured page state without a raw full-text field. After privacy confirmation, it can receive sanitized evidence sentences that explain each signal.” |
