...
- 자동 테스트 41개, 프로덕션 빌드, 배포 의존성 감사 통과
...
최근 확인 결과는 테스트 41개 통과, TypeScript + Vite 빌드 통과, 배포 의존성 취약점 0개입니다.

## 알려진 제한

- WebMCP API와 대회 제출 규칙은 구현·제출 시점에 최신 공식 자료로 재확인해야 합니다.
- Git 기능이 없는 일반 채팅 AI는 코드 제안만 가능하고 저장소 작업은 할 수 없습니다.
- 저장소 공개 전환, 공개 URL 검증, 실제 데모 영상 녹화·공개 업로드, Devpost 최종 제출은 아직 완료되지 않았습니다.
