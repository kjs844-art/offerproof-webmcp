...
| 1:38–1:58 | Call `get_case_summary`, then try `build_verification_plan` before enabling agent changes. Show the rejection. | “The summary excludes a raw full-text field while returning sanitized signal evidence. When the client tries to change the checklist before permission is granted, OfferProof rejects it. Read access does not silently become write access.” |
